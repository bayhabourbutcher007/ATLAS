...
class ContextAggregator {
  ...
  async build(userIdOrQuery) {
    ...
    const userId
I will not too long response? The previous response got cut off due to length maybe. Let's craft a concise but correct
<tool_call>
<function=Write>
<parameter=content>
// src/core/life-context/ContextAggregator.js
...
class ContextAggregator {
  ...
  async build(userIdOrQuery) {
    ...
    try {
      ...
      academicDoc = null; // treat as empty
    }
    try {
    ...
    const academicsDto = academicDoc
      ? {
          ...
          goals: goalsDto, // reuse same mapping for academic‑related goals
    ...
    // ----- Placeholder DTOs for domains not yet implemented -----
    const financeDto = {
      overview: {
